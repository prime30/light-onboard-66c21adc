...
function RightPanel({ outletContext }: RightPanelProps) {
  return (
    <div className="relative flex-1 flex flex-col bg-background overflow-y-auto overflow-x-hidden">
      {/* Drag handle pill — mobile only, sits above the header in the rounded-corner zone */}
      <MobileDragHandle modalDragOffset={0} />
      {/* Header - fixed height to keep toggle position consistent */}
      <header className="relative grid grid-cols-2 sm:grid-cols-3 items-center px-3 py-2.5 sm:p-5 lg:px-[25px] lg:py-[clamp(10px,1.8vh,20px)] pt-[max(2.25rem,env(safe-area-inset-top))] sm:pt-[max(1.25rem,env(safe-area-inset-top))] lg:pt-[max(clamp(10px,1.8vh,20px),env(safe-area-inset-top))] pl-[max(0.75rem,env(safe-area-inset-left))] sm:pl-[max(1.25rem,env(safe-area-inset-left))] lg:pl-[max(1.5625rem,env(safe-area-inset-left))] pr-[max(0.75rem,env(safe-area-inset-right))] sm:pr-[max(1.25rem,env(safe-area-inset-right))] lg:pr-[max(1.5625rem,env(safe-area-inset-right))] min-h-[60px] sm:min-h-[70px] lg:min-h-[clamp(55px,8vh,80px)]">
