...
Deno.serve(async (req: Request) => {
  ...
  if (isDisposableEmail(parseResult.data.email)) {
    ...
    if (submittedPassword && shopifyCustomerId) {
      try {
        // Read auto-approval flag from app_settings (service role).
        ...
        if (autoApprovalEnabled) {
          ...
          if (shopifyDomain && shopifyAdminToken) {
            // Step 1: fetch the customer's account_activation_url. Shopify
            // exposes this on the Admin API customer object after creation.
